class PersonalWebsiteEditor {
    ...
    async loadPublicUserData(userId) {
        if (userId !== 'user1') {
        ...
        try {
            ...
            if (userData) {
                this.personalAvatar.src = userData.avatar || this.avatarImg.src;
                this.personalName.textContent = userData.name || '用戶';
                this.personalWebsites.innerHTML = '';
                if (userData.websites) {
                    userData.websites.forEach((website, index) => {
            ...
            this.personalName.textContent = '無法載入數據';
            this.personalWebsites.innerHTML = '<p style="color: #666; text-align: center;">請檢查網路或稍後再試</p>';
        }
    }
    ...
    generateQRCode() {
        alert(`QR碼生成功能開發中！\n\n網址：${this.shareUrl.value}\n\n你可以手動分享這個網址，或使用瀏覽器的分享功能。\n\n請將以下數據複製到 data.json 以更新公開內容：\n${JSON.stringify(this.loadData(), null, 2)}`);
    }
}
